...
impl<M: RefMetadata> Editor<'_, '_, M> {
    ...
    fn integration(&self, nodes: &HashSet<Selector>) -> Result<Integration> {
        ...
        let from_target_sha: HashSet<Selector> = match self
            .workspace
            .graph
            .project_meta
            .target_commit_id
        ...
        for ref_name in reference_names.values() {
            ...
            for selector in self.reachable_from(remote_selector)? {
        ...
        // Sort so the similarity LUT's ambiguity resolution (first-inserted wins)
        // is deterministic: `reachable_from` and the `reference_names` HashMap
        // yield commits in an arbitrary order, which would otherwise let the
        // displayed `LocalAndRemote(remote_id)` vary between runs.
        remote_only_ids.sort();
